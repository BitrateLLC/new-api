...
const TwoFASetting = ({ t }) => {
  ...
  const BackupCodesDisplay = ({ codes, title, onCopy }) => {
    return (
      <Card className='!rounded-2xl' style={{ width: '100%', background: 'var(--hp-card)', border: '1px solid var(--hp-border)', boxShadow: 'var(--hp-shadow)' }}>
        <div className='space-y-3'>
          <div className='flex items-center justify-between'>
            <Text strong style={{color:'var(--hp-text)'}}>
              {title}
            </Text>
          </div>
          ...
          <div className='grid grid-cols-1 sm:grid-cols-2 gap-2'>
            {codes.map((code, index) => (
              <div key={index} className='rounded-lg p-3' style={{ background: 'var(--hp-fill, var(--semi-color-fill-0))' }}>
                <div className='flex items-center justify-between'>
                  <Text
                    code
                    className='text-sm font-mono' style={{color:'var(--hp-text)'}}
                  >
                    {code}
                  </Text>
          ...
          <Button
            ...
            icon={<IconCopy />}
            onClick={onCopy}
            className='!rounded-lg w-full' style={{ background: 'var(--hp-accent)', borderColor: 'var(--hp-accent)' }}
          >
            {t('复制所有代码')}
          </Button>
        </div>
      ...
      <>
        ...
        {currentStep < 2 ? (
          <Button
            ...
            theme='solid'
            onClick={() => setCurrentStep(currentStep + 1)}
            style={{ borderRadius: '8px', transition: 'all 0.2s ease', background: 'var(--hp-accent)', borderColor: 'var(--hp-accent)' }}
          >
            {t('下一步')}
          </Button>
        ) : (
    ...
    return (
      <>
        ...
        <Button
          ...
          style={{ borderRadius: '8px', transition: 'all 0.2s ease', background: 'var(--hp-accent)', borderColor: 'var(--hp-accent)', opacity: 0.85 }}
        >
          {t('确认禁用')}
        </Button>
      </>
  ...
  return (
    <>
      <Card className='!rounded-2xl w-full' style={{ background: 'var(--hp-card)', border: '1px solid var(--hp-border)', boxShadow: 'var(--hp-shadow)', transition: 'all 0.2s ease' }}>
        <div className='flex flex-col sm:flex-row items-start sm:justify-between gap-4'>
          <div className='flex items-start w-full sm:w-auto'>
            <div className='w-12 h-12 rounded-full flex items-center justify-center mr-4 flex-shrink-0' style={{background:'var(--hp-bg-soft)'}}>
              <IconShield
                size='large'
      ...
      <Modal
        title={
          <div className='flex items-center'>
            <IconShield className='mr-2' style={{color:'var(--hp-sub)'}} />
            {t('设置两步验证')}
          </div>
        }
        ...
        {setupData && (
          <div className='space-y-6'>
            ...
            <div className='rounded-xl'>
              {currentStep === 0 && (
                <div>
                  <Paragraph className='mb-4' style={{color:'var(--hp-sub)'}}>
                    {t(
                      '使用认证器应用（如 Google Authenticator、Microsoft Authenticator）扫描下方二维码：',
                    )}
                  </Paragraph>
                  <div className='flex justify-center mb-4'>
                    <div className='p-4 rounded-lg shadow-sm' style={{background:'var(--hp-card)'}}>
                      <QRCodeSVG value={setupData.qr_code_data} size={180} />
                    </div>
                  </div>
                  <div className='rounded-xl p-3' style={{ background: 'var(--hp-fill, var(--semi-color-fill-0))', border: '1px solid var(--hp-border)' }}>
                    <Text className='text-sm' style={{ color: 'var(--hp-text)' }}>
                      {t('或手动输入密钥：')}
      ...
      <Modal
        title={
          <div className='flex items-center'>
            <IconAlertTriangle className='mr-2' style={{ color: 'var(--semi-color-danger)' }} />
            {t('禁用两步验证')}
          </div>
        }
        ...
        <div className='space-y-6'>
          ...
          <div className='space-y-4'>
            <div>
              <Text
                strong
                className='block mb-2' style={{color:'var(--hp-text)'}}
              >
                {t('禁用后的影响：')}
              </Text>
              <ul className='space-y-2 text-sm' style={{color:'var(--hp-sub)'}}>
      ...
      <Modal
        title={
          <div className='flex items-center'>
            <IconRefresh className='mr-2' style={{color:'var(--hp-sub)'}} />
            {t('重新生成备用码')}
          </div>
        }
            ...
            <>
              ...
              <Space vertical style={{ width: '100%' }}>
                <div className='flex items-center justify-center gap-2'>
                  ...
                  <Text
                    strong
                    className='text-lg' style={{color:'var(--hp-text)'}}
                  >
                    {t('新的备用码已生成')}
                  </Text>
                </div>
                <Text className='text-sm' style={{color:'var(--hp-sub)'}}>
                  {t('旧的备用码已失效，请保存新的备用码')}
                </Text>
